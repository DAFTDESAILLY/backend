...
export class GradesService {
  ...
  async createBatch(
    createDto: CreateGradeDto[] | { grades: CreateGradeDto[] } | CreateGradeDto,
  ) {
    ...
    if (Array.isArray(createDto)) {
      dtoArray = createDto;
    } else if ('grades' in createDto && Array.isArray(createDto.grades)) {
      dtoArray = createDto.grades;
    } else {
      // If it's a single object, wrap it in an array
      dtoArray = [createDto as CreateGradeDto];
    }

    console.log('📊 Procesando array de:', dtoArray.length, 'calificaciones');

    const results: Grade[] = [];
    const errors: Array<{ dto: CreateGradeDto; error: string }> = [];
